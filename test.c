...
void complete_dd_task(struct dd_task task);
...
int main(void) {
	...
	xTaskCreate(xDeadlineTask1Generator, "DeadlineTask1Generator", configMINIMAL_STACK_SIZE, (void *) testBench, 2, NULL);
	xTaskCreate(xDeadlineTask2Generator, "DeadlineTask2Generator", configMINIMAL_STACK_SIZE, (void *) testBench, 2, NULL);
	xTaskCreate(xDeadlineTask3Generator, "DeadlineTask3Generator", configMINIMAL_STACK_SIZE, (void *) testBench, 2, NULL);
	...
	completed_message_queue = xQueueCreate(2, structSize);
...
void complete_dd_task(struct dd_task task) {
	...
	xQueueSend(completed_message_queue, &task, 0);
	// Delete task
	vTaskDelete(NULL);
}
...
void xUserTasks(void *pvParameters) {
	// Get times
	struct dd_task *current_task = (struct dd_task*) pvParameters;

	int executionTime = current_task->execution_time;
...
void xDeadlineScheduler(void *pvParameters) {
	...
	while(1) {
		...
		if(xQueuePeek(message_release_queue, &user_task, 0)) continue;

		// Check if there is another active_task to start
		if(active_task_list->next_task != NULL) {
			// Get head of list
			...
			xTaskCreate(xUserTasks, "UserDefinedTask", configMINIMAL_STACK_SIZE, &task_to_run, 4, &task_to_run.t_handle);
		}

		// Check if we have received completed message
		struct dd_task completed_task;
		if(xQueueReceive(completed_message_queue, &completed_task, 1)) {
			...
			while(finished_task->task.absolute_deadline != completed_task.absolute_deadline && finished_task->task.execution_time != completed_task.execution_time && finished_task->task.task_id != completed_task.task_id) {
